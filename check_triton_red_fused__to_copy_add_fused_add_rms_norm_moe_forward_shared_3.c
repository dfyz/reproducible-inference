...
struct InOuts {
    bf16 residual[ROWS][COLS];
    bf16 hidden[ROWS][COLS];
...
int main(int argc, char** argv) {
    ...
    for (size_t rr = 0; rr < ROWS; ++rr) {
        float thread_acc[COLS/ELEMS_PER_THREAD] = {0};
        for (size_t cc = 0; cc < COLS; cc += ELEMS_PER_THREAD) {
            float val0 = to_float(in_outs->residual[rr][cc + 0]) +
                         to_float(in_outs->hidden  [rr][cc + 0]);
            float val1 = to_float(in_outs->residual[rr][cc + 1]) +
                         to_float(in_outs->hidden  [rr][cc + 1]);
        ...
        float warp_acc[COLS/ELEMS_PER_THREAD/WARP_THREADS] = {0};
        for (size_t start = 0; start < COLS/ELEMS_PER_THREAD; start += WARP_THREADS) {
        ...
        for (size_t to_xor = 4; to_xor > 0; to_xor >>= 1) {
            for (size_t cc = 0; cc < to_xor; ++cc) {
        ...
        for (size_t cc = 0; cc < COLS; ++cc) {
            float val = to_float(in_outs->residual[rr][cc]) +
                        to_float(in_outs->hidden  [rr][cc]);
